...
An F-shaped source graph: one continuous spine, two connections, two mint nodes. Navy provides contrast; mint highlights relationships. All SVG marks are original vector artwork, with no external resources. These assets are covered by the repository’s MIT license.
...
Colors: navy `#12283F`, mint `#7EE0C3`, darker mint for light surfaces `#22977A`, off-white `#F5F8FC`. Leave at least one node diameter of clear space around the mark. Keep its proportions. Use the icon at small sizes, and the wordmark when there is enough room to read the name.
