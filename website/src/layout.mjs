...
export function layout(page, config) {
  ...
  const schema={'@context':'https://schema.org','@type':'WebPage',name:page.title,description:page.description,url:canonical,isPartOf:{'@type':'WebSite',name:'Fehm',url:config.siteUrl}};
  return `<!doctype html>
...
<title>${escape(title)}</title><meta name="description" content="${escape(page.description)}"><meta name="robots" content="${noindex ? 'noindex, nofollow' : 'index, follow'}">
<link rel="canonical" href="${escape(canonical)}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="preload" href="/fonts/manrope.woff" as="font" type="font/woff" crossorigin><link rel="stylesheet" href="/styles.css">
...
<a class="skip" href="#main">Skip to content</a><header class="site-header"><div class="header-inner"><a class="brand" href="/" aria-label="Fehm home"><svg viewBox="0 0 28 28" aria-hidden="true"><path d="M5 22V6h18M5 14h13M13 6v16"/></svg>fehm</a><nav aria-label="Main navigation">${navigation.map(([url,label])=>`<a href="${url}" ${page.path.startsWith(url)?'aria-current="page"':''}>${label}</a>`).join('')}</nav><a class="header-cta" href="/getting-started/" data-event="install_guide_click">Get started <span aria-hidden="true">↗</span></a></div></header>
<main id="main" class="shell ${page.kind?'reading-shell':''}">${page.kind?article(page):page.body}</main>
${page.kind?'':`<section class="closing shell"><p class="eyebrow">A little more context goes a long way.</p><h2>Get to know <em>your code.</em></h2><a class="button" href="/getting-started/" data-event="install_guide_click">Start with Fehm ↗</a></section>`}
